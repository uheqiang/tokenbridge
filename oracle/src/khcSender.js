...
const { sendTx, sendTxToKhc } = require('./tx/sendTx')
  ...
  try {
    ...
    await syncForEach(txArray, async job => {
      ...
      try {
        if (isResend) {
          // TODO START
          const tx = await web3Fallback.getTransaction(job.txHash)
          ...
          logger.info(`Transaction ${job.txHash} was not mined`)
        }
        ...
        // nonce++
        logger.info({ eventTransactionHash: job.transactionReference, generatedTransactionHash: txHash },
          `Tx generated ${txHash} for event Tx ${job.transactionReference}`)
      } catch (e) {
        logger.error({ eventTransactionHash: job.transactionReference, error: e.message },
          `Tx Failed for event Tx ${job.transactionReference}.`, e.message)
        ...
        if (message.includes('insufficient funds')) {
          ...
          // todo
          // const currentBalance = await web3Instance.khc.getBalance(ORACLE_VALIDATOR_ADDRESS)
